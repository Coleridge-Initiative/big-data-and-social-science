<!--
% todo: add example to table, work on comments
--> 
...
<!-- Comment: While okay, I'm not 100% happy with "tabular" data. Tabular data is already the product of some analysis; what is meant here is more something like regular, plain old raw data that consists of numbers. However, I don't a better term. -->
...
<!-- as unstructured. Even more, you appear to equate the two, i.e. you imply that -->
<!-- unstructured data is always text data and vice versa. At the least, to solve this, -->
<!-- you could write in this footnote something like "For short, we will refer to text data -->
<!-- as unstructured in this chapter." -->
...
-   **Adding Linguistic Features**: This step is only needed when the problem requires deeper linguistic analysis. For example, when trying to understand the structure of a sentence, we can use a part of speech tagger <!-- Comment: totally unclear to me what you mean here. Do you mean the Microsoft program, but then why the "a" and no capitalization? --> to tag words with their corresponding part of speech (noun phrase for example) and use a statistical parser to generate what’s called a parse tree that shows relationships between different components of a sentence. 
...
their parts.
<!-- Comment: While what you write is correct, this is not a complete characterization of n-grams, as it does not have be n words. It can also be about, e.g. n letters. As n-grams involving letters are also important, I suggest you think about re-writing this chunk or re-naming it from n-grams. -->
...
<!-- Comment: You appear to write the following sentences from the perspective of having multiple documents, -->
<!-- and each document constituting a row in the matrix. If so, please make that clear what example you're implicitly using here. -->
<!-- Comment: You mix the words column and cell; e.g. in "give each column a binary 0 or 1 value": you probably don't mean that the whole column should all 1s or 0s in it, rather you mean that a particular cell (of a column) could take on either 1 or 0 as value, i.e. that column consists of 1s and 0s. -->
...
tokens (where a token can be a word or a phrase).<!-- Comment: I would have preferred that you defined this term above, e.g. in "Tokenization" rather than here, off-hand. --> One simple approach
...
<!-- Comment: Please, define N! Is it the Cardinality of D, i.e. N := |D|? Then introduce it in the first sentence, à la "... in the corpus $D$ with size/cardinality N, ...". -->
...
<!-- Comment: what's a term vector? -->
<!-- Comment: why do you * here instead of \times, as in the previous formula? -->
<!-- Comment: Might also make sense to make clear the definition of the dot product for vectors, i.e. spell it out with its components? -->
...
<!-- Comment: (verbally) define what a synset is. -->
...
<!-- Comment: doc should probably be d. -->
...
<!-- Comment: "Evaluating “Find Similar” Methods:" should probably some type of heading? -->
...
<!-- Comment: It's totally unclear what $K$ is. Also, that's a very crude way to describe posterior inference.-->
...
<!-- Comment: Why is called Listing 7.4? There are no Listings 7.1-7.3? Also, this chapter 8 not chapter 7. -->
