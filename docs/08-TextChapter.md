...
when dealing with these types of data is how to efficiently analyze it
just like we analyze traditional tabular data. For example, when
analyzing survey responses or electronic health records data, both of
which contain narrative text (from the respondents and medical
...
specific that go deeper into the syntax of the document, paragraph,
...
lawyers where they are looking at a case file and want to find all
...
key challenge here it to define what makes two documents similar and
...
$w_{t,b}$ are term weights in two vectors, respectively.
...
and machine learning, we will restrict our focus to a single methods
...
analysis [@landauer-97]. Blei and Lafferty [@blei-09] provide a more
