...
The module, available natively in Jupyter Python notebooks, is a useful
...
checking the actual HTML that was returned. A code means success and
...
series of s, each of which corresponds to one investigator. These have
the class attribute . Again, there is nothing obvious about finding
...
variable ) and then find the element that holds the information on
investigators (). As this element is unique on the page (I checked using
my web browser), we can use the find method. We then process that (using
) to create an iterator object that contains each of the page segments
detailing a single investigator ().
...
The number of results is found in a with the class "view-headers" as a
...
\exampend{Page views and downloads}
...
\exampend{Discussions on social media}
...
and a data "query," in this case made up of a "field" () and an
identifier (the DOI ). The Crossref API provides information about the
...
Once again the Python library provides a series of convenience functions
...
The object that the library has created has a range of useful
...
perspective of our script) converted to a Python dictionary. The module
...
the object is a list populated with publications (which are also
...
The module is a wrapper for the Lagotto API, which is served from a
...
Each can be accessed by name from a dictionary called "sources." The
...
##### Provide evidence that ... {#provide-evidence-that .unnumbered}
...
##### What flavor is it? {#what-flavor-is-it .unnumbered}
...
Section [1.3](#sec:4-2){reference-type="ref" reference="sec:4-2"} is
...
Section [1.9.4](#sec:4-7.1.4){reference-type="ref"
