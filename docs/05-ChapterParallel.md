...
domain name. Thus, we adopt a strategy of first grouping all award
...
replication is the primary mechanism for fault tolerance in Hadoop.
...
1. a *compute cluster* with substantial computing
...
-   **Performance**: Hadoop has proven to be a scalable implementation that can run on
    ...
    relatively high job setup overhead and suboptimal running time. An
...
estate market. The data^[http://www.consumerfinance.gov/hmda/learn-more], 
which are a product of the Home Mortgage Disclosure Act (HMDA), 
highlight key attributes that function
