...
reference="chap:intro"}. Although the consequences are well understood,
...
In Figure [1.1](#fig:10-1){reference-type="ref" reference="fig:10-1"}, a
...
\centering
![A typical rectangular data file
format[]{label="fig:10-1"}](ChapterError/figures/fig1){#fig:10-1}
...
[1.1](#fig:10-1){reference-type="ref" reference="fig:10-1"}. However, in
...
reference="chap:web"}), (b) extract, transform and load (ETL), and (c)
analysis (see, for example,
Chapter [\[chap:text\]](#chap:text){reference-type="ref"
reference="chap:text"}). As noted there, this mapping of the process is
...
Figure [1.2](#fig:10-2){reference-type="ref" reference="fig:10-2"}
...
\centering
![Big data process
map[]{label="fig:10-2"}](ChapterError/figures/fig2){#fig:10-2}
...
[1.4.2](#sec:10-4.2){reference-type="ref" reference="sec:10-4.2"}
considers three common types of analysis when data lack veracity:
...
[1.3](#fig:10-3){reference-type="ref" reference="fig:10-3"} is an
...
\centering
![An illustration of coincidental correlation between two variables:
stork die-off linked to human birth decline
[@sies1988new][]{label="fig:10-3"}](ChapterError/figures/fig3){#fig:10-3}
...
### Errors resulting from lack of veracity {#sec:10-4.2}

The previous section examined some of the issues big data analysts face
as either $N$ or $p$ in Figure [1.1](#fig:10-1){reference-type="ref"
reference="fig:10-1"} becomes extremely large. When row, column, and
...
[1.4.2.4](#sec:10-4.2.4){reference-type="ref" reference="sec:10-4.2.4"},
...
#### Misclassification and rare classes {#sec:10-4.2.3}
...
Section [1.4.2.2](#sec:10-4.2.2){reference-type="ref"
reference="sec:10-4.2.2"} and Bayes' rule, we can derive the PPV as
$$\begin{aligned}
...
In Section [1.4.1](#sec:10-4.1){reference-type="ref"
reference="sec:10-4.1"}, we considered the problem of incidental
...
Section [1.4.2.1](#sec:10-4.2.1){reference-type="ref"
reference="sec:10-4.2.1"} and begin with theoretical results that help
...
[1.3](#fig:10-3){reference-type="ref" reference="fig:10-3"}. Therefore,
...
[1.4](#fig:10-4){reference-type="ref" reference="fig:10-4"} illustrates
...
\centering
![Correlation as a function of sample size for $\rho_{\mu
\vert cd} = 0.5$ and
$R_c = R_d = 0.5$[]{label="fig:10-4"}](ChapterError/figures/fig4){#fig:10-4}
...
Figure [1.4](#fig:10-4){reference-type="ref" reference="fig:10-4"} with
$n_c = n_d =
...
[1.5](#fig:10-5){reference-type="ref" reference="fig:10-5"} shows the
...
\centering
![Correlation as a function of sample size for $\rho_{\mu \vert
cd} = 0.5$, $R_c = R_d = 0.5$, and
$\rho_c = \rho_d = 0.25$[]{label="fig:10-5"}](ChapterError/figures/fig5){#fig:10-5}
...
#### Regression analysis {#sec:10-4.2.5}
...
in Figure [\[fig:10-6\]](#fig:10-6){reference-type="ref"
reference="fig:10-6"}, which are based upon contrived data. The
...
\centering
![image](ChapterError/figures/fig6)
...
as noted in Section [1.1](#sec:10-1){reference-type="ref"
reference="sec:10-1"}, and there is limited opportunity to address
...
In survey collections, data processing may involve data entry, coding
...
To complement fully automated micro-editing, big data editing usually
...
Section [1.4.2.1](#sec:10-4.2.1){reference-type="ref"
reference="sec:10-4.2.1"} regarding random noise, we know that the data
...
reference="chap:viz"}, the tableplot has the ability to summarize a
...
[\[fig:10-7\]](#fig:10-7){reference-type="ref" reference="fig:10-7"}
shows an example. The key idea is that micro-aggregates of two related
...
\centering
![image](ChapterError/figures/fig7){width="\textwidth"}
...
The tableplot in Figure [\[fig:10-7\]](#fig:10-7){reference-type="ref"
reference="fig:10-7"} is taken from Tennekes and de Jonge
...
[\[fig:10-7\]](#fig:10-7){reference-type="ref" reference="fig:10-7"} was
...
This review of big data errors should dispel the misconception that the
